...
let goods = [
  ...
  function showArray(array){
    ...
    array.forEach((prod) => {
        ...
        <p>Product price: ${prod.product_price}</p>
        <p>Product desc: ${prod.product_description}</p>
        <p>Store name: ${prod.store_name}</p>
        <p>Store address: ${prod.store_address}</p> `
  ...
  function filterForPrice(array) {
    let filteredPrice = array.filter(prod => prod.product_price > 35 && prod.product_price < 150)
    showArray(filteredPrice)
  }
  filterForPrice(goods)

  function filterForName(array, name){
    let filteredName = array.filter(prod => prod.product_name.toLowerCase().includes(name.toLowerCase()))
    showArray(filteredName)
  }
  filterForName(goods,"Qadın ətri")
